...
import { drawInitialBars } from "./drawBars.js";
import installBrushAndZoom from "./brushAndZoom.js";
...
import { initSelectFile } from "./selectFile";
...
export function initChart(selectedFile) {
  ...
  //drawDetailBars(chart, tokens, annos, chunks);
  //drawOverviewBars(chart, tokens, annos, chunks);
  //
  initTextArea(chart);
  initEvents(chart);
  initFunctions(chart);
  installBrushAndZoom(chart);
  dragChart(chart);
  initLens(chart);
  initIndicator(chart);

  initElemrntAddres(chart);
  initStateController(chart);
  initFilters(chart);
  // d3.json(selectedFile, function (data) {
  //   // d3.json("../data/werther.json", function (data) {
  //
